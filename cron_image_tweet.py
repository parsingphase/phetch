...
def parse_cli_args() -> argparse.Namespace:
    ...
    group.add_argument(
        "--source-flickr-download-dir",
        help="Directory of coded flickr downloads to use as source",
    )
    group.add_argument(
        "--source-flickr-file-list",
        help="File containing list of Flickr photo dates/ids",
    )
    parser.add_argument(
        "--dry-run", help="Prepare the tweet but don't send it", action="store_true"
    )
    parser.add_argument(
        "--for-date", help="Generate a tweet for a specific day (YYYYmmdd format)"
    )
...
def post_tweet_from_schedule(
    ...
    if due_photo:
        tweet: SimpleTweet = build_tweet_by_flickr_photo_id(
            due_photo["photo_id"], hashtag
        )

                    ...
                    if "native_page_url" in photo:
                        ...
                        if native_page_url:
                            ...
                            if flickr_title != species:
                                ignore_description = (
                                    len(inat_description) == 0
                                ) or "Species TBD" in inat_description
                                replaceable_title = (
                                    "4Y6A" in flickr_title
                                    or "?" in flickr_title
                                    or flickr_title in always_replace
                                )
                                species_name_longer = len(species) >= len(flickr_title)
                                if last_was_dot:
                                    print("")
                                if replaceable_title or (
                                    species_name_longer and ignore_description
                                ):
                                    print(
                                    ...
                                    flickr.photos.setMeta(
                                        photo_id=photo_id, title=species
                                    )
                                    flickr.photos.addTags(
                                        photo_id=photo_id,
                                        tags=f'"fixed:oldTitle={flickr_title}"',
                                    )
                                    print(" OK")
                                else:
                                    desc_block = (

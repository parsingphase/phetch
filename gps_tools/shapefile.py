...
class ShapefileLocationFinder:
    ...
    def __init__(self, shapefile_path: str, place_name_field: str) -> None:
        ...
        self.gps_point_transformer = pyproj.Transformer.from_crs(
            EPSG_DATUM["WGS84"], self.coord_reference, always_xy=True
        )
        ...
        for i in range(1, len(shapes)):
            ...
            try:
                ...
                if poly.contains(point):
                    ...
                    if (
                        record_matches(record, "GIS_Src", "data.georgiaspatial.org")
                        or record_matches(record, "Loc_Nm", "FLAP")
                        or record_matches(record, "Loc_Nm", "State Trust Land")
                        or record_matches(record, "Unit_Nm", "Field Office")
                        or record_matches(record, "Des_Tp", "MIL")
                        or record_matches(record, "Pub_Access", "XA")
                        or record[self.place_name_field] == "Usbr"
                        or record[self.place_name_field] == "Park"
                    ):

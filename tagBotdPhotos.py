...
from phetch_tools.social import ScheduledId, scan_file_for_coded_filenames
...
def run_cli():
    files_2021: List[ScheduledId] = scan_file_for_coded_filenames(
        Path("potd_schedules.lnk/2021.txt")
    )
    photos_2021 = [file["photo_id"] for file in files_2021]

    files_2022: List[ScheduledId] = scan_file_for_coded_filenames(
        Path("potd_schedules.lnk/2022.txt")
    )
...
def tag_files(
    flickr: flickrapi.FlickrAPI, tag: str, remove_tag: str, photo_ids: List[str]
):
